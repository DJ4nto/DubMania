  ...
  select jsonb_build_object(
    ...
    (
      select jsonb_build_object(
        ...
        'recordingDeadlineAt',
          r.recording_deadline_at,
        'reviewDeadlineAt',
          r.review_deadline_at,
        'finalPlaybackAt',
          r.final_playback_at,
      ...
      (
        select jsonb_agg(
          jsonb_build_object(
            ...
            'startOffsetMs',
              recording.start_offset_ms,
            'endOffsetMs',
              recording.end_offset_ms,
            'createdAt', recording.created_at,
            'validatedAt',
              recording.validated_at
          )
          order by
            recording.player_id,
            recording.attempt
    ...
    coalesce(
      (
        select jsonb_agg(
          played.catalog_video_id
          order by played.played_at
        )
        from public.played_videos played
        where played.lobby_id = l.id
      ),
      '[]'::jsonb
    ),
    'serverTime',
